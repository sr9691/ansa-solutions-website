...
function renderButtons() {
  ...
  row.innerHTML = `
    <button type="button" class="btn btn-secondary" onclick="prevSection()" ${isFirst ? 'disabled' : ''}>
      ← Back
    </button>
    <div style="display:flex;gap:10px;align-items:center">
      <span style="font-size:12px;color:var(--muted)">${Math.round(((currentSection + 1) / totalSections) * 100)}% complete</span>
      ${isLast
        ? '<button type="button" class="btn btn-submit" onclick="submitForm()">Submit Assessment →</button>'
